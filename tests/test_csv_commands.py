#pylint: disable=redefined-outer-name,consider-using-f-string,invalid-name,unused-variable
'''Test csv commands'''
import os
...
def test_load_csv_command(mocker, mock_app):
    ...
    LoadCsvCommand().execute('test.csv', mock_app)
...
def test_load_csv_command_file_not_found(mocker, mock_app):
    ...
    with patch('logging.error') as mock_logging_error:
        LoadCsvCommand().execute('non_existent_file.csv', mock_app)
...
def test_load_csv_command_invalid_file_type(capfd, mock_app):
    ...
    LoadCsvCommand().execute('test.py', mock_app)
    out, err = capfd.readouterr()
